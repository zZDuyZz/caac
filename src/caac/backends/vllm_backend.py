...
class VLLMBackend:
    ...
    def generate(
        ...
        *,
        ...
        # Fix: seed used to be fixed on EVERY call, so any temperature>0
        # sample was just as deterministic/repeated as greedy (seed=0 pins
        # vLLM's RNG to the same draw for the same prompt+temperature every
        # time). That silently made self_consistency/deepconf/BRANCH's
        # "independent" samples identical copies of one sample. Greedy
        # (temp=0) is unaffected either way -- argmax has no randomness to
        # seed. Only when real diversity is requested (temp>0) do we now let
        # vLLM draw fresh randomness each call.
        params = SamplingParams(
            max_tokens=max_tokens,
            temperature=effective_temp,
            logprobs=self.top_logprobs,
            stop=self.stop,
            seed=self.seed if effective_temp == 0.0 else None,
        )
